...
def sampling(  # pylint: disable=too-many-locals
    ...
    :param before_step_fn: a callback function before each step, defaults to None
    ...
    :param solver: integration method, either "heun" or "euler"
    :type solver: str, optional
    :return: final sample or whole trajectory
    """
    solver = solver.lower()
    ...
    for cur_t, next_t in loop:
        ...
        if solver == "euler":
            # Euler update
            xt_next = cur_x + (next_t - cur_t) * epsilon_1

        else:  # solver == "heun"
            # Predictor
            xt_pred = cur_x + (next_t - cur_t) * epsilon_1

            # Corrector evaluation
            x0_2, loss_info_2, traj_info_2 = x0_pred_fn(xt_pred, next_t)
            epsilon_2 = (xt_pred - x0_2) / next_t

            # Heun update
            xt_next = cur_x + (next_t - cur_t) * (epsilon_1 + epsilon_2) / 2

            # Keep second-call info, since it corresponds to the corrected endpoint
            loss_info = loss_info_2
            traj_info = traj_info_2
        ...
        if return_traj:
            ...
            traj["xt"].append(running_x.detach().cpu())
